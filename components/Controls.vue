...
const recodingActive = computed(() => recording.value)
const emit = defineEmits(["play", "reset", "record"])
</script>
    ...
    <div class="controls vstack gap-2">
      ...
      <section class="panel controls__timeline gap-5 hstack space">
        <span class="hstack gap-2">
          <button v-if="playing" class="play-pause" @click="reset">Stop</button>
          <button v-else class="play-pause" @click="play">Play</button>
          <button @click="record" class:recodingActive>
        ...
        <header class="hstack gap-2">
          ...
          <a href="/about" class="btn-plain q-link" aria-label="About this app">
            <svg aria-hidden="true" class="q-icon" xmlns="http://www.w3.org/2000/svg" width="32" height="32"
              viewBox="0 0 24 24">
        ...
        <label for="title" class="vstack gap-3">
          <div class="size-color hstack gap-3 space">
            ...
            <label for="titleSize" class="hstack gap-3">
              <span>Font Size</span>
              <input id="titleSize" type="range" min="20" max="100" v-model="sizeTitle" />
            </label>
            <label for="fg" class="color-input-fg" aria-label="foreground-color"
              :style="`--fg: ${fg}; border-radius: 50%`">
              <input class="visually-hidden fg" type="color" id="fg" v-model="fg" />
            </label>
          </div>
          <textarea id="title" class="textInput" v-model="title" />
        </label>
      </section>
        ...
        <label for="subtitle" class="vstack gap-3">
          <div class="size-color hstack gap-3 space">
            ...
            <label for="subtitleSize" class="hstack gap-3">
              <span>Font Size</span>
              <input id="subtitleSize" type="range" min="20" max="100" v-model="sizeSubtitle" />
            </label>
            <label for="fg2" class="color-input-fg" aria-label="Foreground color" :style="`--fg: ${fg}; border-radius:
              50%`">
              <input class="visually-hidden fg" type="color" id="fg2" v-model="fg" />
            </label>
          </div>
          <textarea id="subtitle" class="textInput textInput--sm" v-model="subtitle" />
        </label>
      </section>
      ...
      <section class="panel controls__bgcolor hstack gap-3">
        <label for="bgOpacity" class="flex-fill hstack gap-3">
          ...
          <input class="flex-fill" id="bgOpacity" type="range" min="0" max="1" step=".01" v-model="bgOpacity" />
        </label>
        <label for="bg" class="color-input-bg" aria-label="Background color" :style="`--bg: ${bg}; border-radius:
              50%`">
          <input class="visually-hidden bg" type="color" id="bg" v-model="bg" />
        </label>
      </section>
